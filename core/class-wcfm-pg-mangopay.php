...
class WCFM_PG_MangoPay
{
	...
	{
		// Check if user_mp_status is 'individual' and unset user_mp_status and user_business_type
		if (isset($input_data['user_mp_status']) && $input_data['user_mp_status'] === 'individual') {
			unset($input_data['user_mp_status'], $input_data['user_business_type']);
		}

		// Check if billing_state is '' then unset billing_state 
		if (isset($input_data['billing_state']) && $input_data['billing_state'] === '') {
		...
		foreach ($input_data as $key => $value) {
			...
			if (array_key_exists($key, $required_fields)) {
				...
				if (empty($value)) {
					$errors[$key] = "Error: {$required_fields[$key]} is required.";
				}
			}
		}
